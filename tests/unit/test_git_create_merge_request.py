...
class TestGitCreateMergeRequest(unittest.TestCase):
    ...
    def test_must_return_merge_request_iid(self, mock_urlopen):
        ...
        actual = git_create_merge_request('https://gitlab.com', 'gitlab_token', 'project_id', ['version_changes'])
    ...
    def test_request_must_contain_body_with_source_branch(self, mock_urlopen):
        ...
        self.assertEqual(json.loads(decoded_data).get('source_branch'), 'master')

    def test_request_must_contain_body_with_target_branch(self, mock_urlopen):
        mock_urlopen.return_value = self.mock_read(str.encode('{"iid": "iid"}'))
        git_create_merge_request('https://gitlab.com', 'gitlab_token', 'project_id', ['version_changes'])
        decoded_data = mock_urlopen.call_args[0][0].data.decode('utf-8')
        self.assertEqual(json.loads(decoded_data).get('target_branch'), 'develop')

    def test_request_must_contain_body_with_title(self, mock_urlopen):
        mock_urlopen.return_value = self.mock_read(str.encode('{"iid": "iid"}'))
        git_create_merge_request('https://gitlab.com', 'gitlab_token', 'project_id', ['version_changes'])
        decoded_data = mock_urlopen.call_args[0][0].data.decode('utf-8')
        self.assertEqual(json.loads(decoded_data).get('title'), 'Automatic merge branch \'master\' into \'develop\'')
    ...
    def test_multiple_changes_request_must_contain_body_with_description(self, mock_urlopen):
        mock_urlopen.return_value = self.mock_read(str.encode('{"iid": "iid"}'))
        git_create_merge_request('https://gitlab.com', 'gitlab_token', 'project_id', ['chng1', 'chng2'])
        decoded_data = mock_urlopen.call_args[0][0].data.decode('utf-8')
