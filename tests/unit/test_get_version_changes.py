...
from ci_helper import get_version_changes
...
class TestGetVersionChanges(BaseTest):
    ...
    @mock.patch('ci_helper.get_merge_request_changes', side_effect=HTTPError('url', 'cde', 'msg', 'hdrs', 'fp'))
    @mock.patch('ci_helper.get_commit_changes')
    ...
    @mock.patch('ci_helper.get_merge_request_changes', return_value=[])
    @mock.patch('ci_helper.get_commit_changes', side_effect=HTTPError('url', 'cde', 'msg', 'hdrs', 'fp'))
    def test_error_on_commit_request_must_raise_http_error(self, mock_get_merge_request_changes,
                                                           mock_get_commit_changes):
        with self.assertRaises(HTTPError):
            get_version_changes('https://gitlab.com', 'gitlab_token', 'project_id', 'commit_sha')

    @mock.patch('ci_helper.get_merge_request_changes', return_value=['changes'])
    @mock.patch('ci_helper.get_commit_changes')
    ...
    @mock.patch('ci_helper.get_merge_request_changes', return_value=[])
    @mock.patch('ci_helper.get_commit_changes', return_value=['changes'])
    ...
    @mock.patch('ci_helper.get_merge_request_changes', return_value=[])
    @mock.patch('ci_helper.get_commit_changes', return_value=[])
